...
#Answer: super high autocorrelation at 5 second fixes due to more data points.
# 
...
#Answer: I think that sample size could affect individuals differently but individual behavior and prey availability could also be factors to consider. 3 and 4 showed the largest differences between methods but 4 has 8 times as many data points as 3, leading me to believe that there could be other factors affecting the difference of methods across individuals. 
#
...
# Answer: Earlier in the year, points are more consistent in center locations (probably nests). Later in the year, individuals are traveling to further and less consistent areas, likely for foraging. 
#

#Which weeks do you remove for each individual:
#Answer:Depending on the research question. If looking for nest locations, points earlier in the year are worth keeping while discarding later points. To obtain a full home range, keeping the later points would be helpful and thinning points during the nesting phase could reduce the computational efforts. 
#

#update the hr_wk tibble by removing those weeks without enough
# data:
## Selecting weeks after the 22nd week. 

hr_wk_updated <- trks.thin %>%  
# we nest by id and week
nest( data = -c(id, wk, sex ) ) %>%
  mutate( n = map_int(data, nrow) ) %>% 
  #remove weeks without enough points
...
ids_updated <- hr_wk_updated %>% 
  group_by( id ) %>% 
  slice( 1 ) %>% 
  dplyr::select( id, sex )

# this way you can loop through each individual
for( i in ids_updated$id ){
  # #filter data for one individual at a time:
  wp <- hr_wk_updated %>% filter( id == ids$id[i] ) %>% 
    #turn range estimates to sf objects, keeping relevant id details
    hr_to_sf( hr_kde, id, sex, wk, n ) %>% 
    #plot with ggplot
    ggplot( . ) +
...
# Are there any seasonal trends in range use for each individual?
#Answer: It looks that thinning data to after the 22nd week shows the areas that adults are traveling post breeding season. Interestingly, some individuals travel long distances away from their nesting areas while others remain closer to home. I wonder if this could be due to a successful nesting attempt?   
#
...
#Answer: It does not appear that males and females have a significant difference in home range sizes. It is interesting to compare the difference of mcp and kde!
# 
...
# Answer: At 50% of their breeding rages, males and females don't differ greatly. This suggests that they both play important roles during the nesting phase. Additionally the smaller 50% ranges compared to the larger 90% ranges could suggest that some individuals have to travel farther for prey than other individuals. 
#
